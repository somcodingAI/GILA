...
class ATT_learner(nn.Module):
    def __init__(self, nlayers, isize, k, knn_metric, i, sparse, mlp_act, updaters_condition,fake_idx, features):
        ...
        self.updaters_condition = updaters_condition
        self.num_samples=features[0]
        self.in_channels = features[1]

        
        
        from torch_geometric.nn.inits import glorot
        if self.updaters_condition == True:

            self.updaters = nn.Parameter(torch.FloatTensor(fake_idx.shape[0], features.shape[1]))
    ...
    def forward(self, features, zero_matrix):
        if self.sparse:
            if self.updaters_condition== True:
                features = self.add_prompt(features, zero_matrix)
