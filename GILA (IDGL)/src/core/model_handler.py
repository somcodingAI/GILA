...
class ModelHandler(object):
    ...
    def __init__(self, config, args):
        ...
        datasets, new_class_num_list = prepare_datasets(config, args)
        self.new_class_num_list = new_class_num_list
        ...
        if config['data_type'] in ('network', 'uci'):
            ...
            # print("self.run_epoch",self.run_epoch)
    ...
    def test(self):
        ...
        bacc = balanced_accuracy_score(gold.cpu(), pred.cpu())
        f1 = f1_score(gold.cpu(), pred.cpu(), average='macro')
        ...
        print(f'bAcc: {bacc}, f1: {f1}, ')

        format_str = "Finished Testing: {}\nTesting time: {}".format(self.dirname, timer.total)
        print(format_str)
        self.logger.write_to_file(format_str)
        self.logger.write_to_file(f'class num :{self.new_class_num_list}')
        self.logger.write_to_file(f'{CM}') 
        self.logger.write_to_file(f'f1: {f1}') 
        self.logger.write_to_file(f'bAcc: {bacc}') 
        ...
        return bacc,f1, self.new_class_num_list
    ...
    def _run_whole_epoch(self, data_loader, training=True, verbose=None, out_predictions=False):
        ...
        while self.config['graph_learn'] and (iter_ == 0 or diff(cur_raw_adj, pre_raw_adj, first_raw_adj).item() > eps_adj) and iter_ < max_iter_:
            ...
            if self.prompt:
                ...
                node_vec = torch.relu(network.encoder.graph_encoders[0](network.encoder.add_updaters(init_node_vec, zero_matrix), cur_adj))
    ...
    def _scalable_run_whole_epoch(self, data_loader, training=True, verbose=None, out_predictions=False):
        ...
        if self.prompt:
            zero_matrix = torch.zeros((features.shape[0]-fake_idx.shape[0],self.config['num_feat'])).to(self.device)
            init_node_vec = network.encoder.add_updaters(init_node_vec, zero_matrix)
            init_anchor_vec, sampled_node_idx = sample_anchors(init_node_vec, network.config.get('num_anchors', int(0.2 * init_node_vec.size(0))), fake_idx=fake_idx)
        ...
        while self.config['graph_learn'] and (iter_ == 0 or diff(cur_node_anchor_adj, pre_node_anchor_adj, cur_node_anchor_adj).item() > eps_adj) and iter_ < max_iter_:
            ...
            if self.prompt:
                zero_matrix = torch.zeros((features.shape[0]-fake_idx.shape[0],self.config['num_feat'])).to(self.device)
                init_node_vec = network.encoder.add_prompt(init_node_vec, zero_matrix)
