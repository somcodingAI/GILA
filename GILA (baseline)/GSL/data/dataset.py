...
import torch
from GSL.utils import sample_mask, to_scipy, to_tensor, to_undirected, get_dataset, normalize_features, get_idx_info, split_semi_dataset
import sys
...
class Dataset:
    ...
    def __init__(self, fake_samples, prompts,imb_ratio, root, name, seed=None, use_mettack=False, ptb_rate=0):
        ...
        if not use_mettack: ##############################################
            self.adj, self.features, self.labels, self.fake_idx = self.load_data(fake_samples, prompts,imb_ratio)
        else:
    ...
    def load_data(self,fake_samples, prompts,imb_ratio):
        print("Loading {} dataset...".format(self.name))
        if self.name in ['cora', 'citeseer', 'pubmed']:
            return self.load_citation_dataset(fake_samples, prompts,imb_ratio)
    ...
    def load_citation_dataset(self,fake_samples, prompts,imb_ratio): #######################################
        ...
        if dataset_name=='citeseer':
            ...
            print(f"---------------------{dataset_name} preprocessing--------------------------")
            dataset = get_dataset(dataset_name, self.root, split_type='public')
            data=dataset[0]
            objects = []
            for i in range(len(names)):
                name = "{}/raw/ind.{}.{}".format(dataset_name,self.name, names[i])
        ...
        ######## Data statistic ##
        if dataset_name=='citeseer':
            data_train_mask, data_val_mask, data_test_mask = train_mask.clone(), val_mask.clone(), test_mask.clone()
        ...
        for i in range(n_cls):
            if imb_ratio < 1 and i > n_cls-1-imb_class_num: #only imbalance the last classes
                # print("n_cls-1-imb_class_num", n_cls-1-imb_class_num, i)
                # new_class_num_list.append(min(int(max_num*(1./imb_ratio)), class_num_list[i]))
        ...
        self.train_mask = torch.Tensor(data_train_mask)
        self.val_mask = torch.Tensor(data_val_mask)
        self.test_mask = torch.Tensor(data_test_mask)      
        self.edge_index = edge_index


        print("data split, train:val:test", self.train_mask.sum(), self.val_mask.sum(),self.test_mask.sum())

        print(f"-----------imb_ratio : {imb_ratio} --------------")
        ...
        if fake_samples == True:
            ...
            self.train_mask = torch.cat([self.train_mask, torch.zeros(len(fake_idx), dtype=torch.bool)])
            self.val_mask = torch.cat([self.val_mask, torch.zeros(len(fake_idx), dtype=torch.bool)])
            self.test_mask = torch.cat([self.test_mask, torch.zeros(len(fake_idx), dtype=torch.bool)])
            
            self.train_mask[-len(fake_idx):] = True
            ...
            #     adj_mask = torch.ones((adj.shape[0],adj.shape[1]), dtype=torch.bool )
            #     # 지워야 하는 edge를 0으로 처리
            #     adj_mask[row,col] = False
            #     adj = adj*adj_mask
            
            #     # fake sample 에 대한 adj추가
            #     print(adj)
            #     n = adj.size(0)
            #     zero_col = torch.zeros(n, num_fake, device=adj.device)
            #     adj = torch.cat([adj, zero_col], dim=1)
            #     zero_row = torch.zeros(num_fake, n + num_fake, device=adj.device)
            #     adj = torch.cat([adj, zero_row], dim=0)
        else:
            fake_idx=None
        ...
        return adj, features, labels, fake_idx
...
###원래함수
    ...
    def load_citation_dataset(self):
        ...
        if self.name =='citeseer':
            ...
            dataset = 'PubMed'
        dataset = get_dataset(dataset, self.root, split_type='full')
        data=dataset[0]
