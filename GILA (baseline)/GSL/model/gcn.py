...
class GCN(nn.Module):
    def __init__(self, num_samples,in_channels, hidden_channels, out_channels, num_layers, dropout, dropout_adj, device, sparse=False, residual=False, activation_last=None, conv_bias=False, bn=False, prompts_condition=False, fake_idx=None):
        ...
        self.prompts_condition = prompts_condition
        ...
        if self.prompts_condition == True:
            print("------------------make prompts------------------")
            self.prompts = nn.Parameter(torch.FloatTensor(fake_idx.shape[0], in_channels))
    ...
    def forward(self, x, adj_t,zero_matrix, return_hidden=False):
        # from IPython import embed; embed(header='gcn.py, line 131')
        Adj = self.dropout_adj(adj_t)
        is_torch_sparse_tensor = Adj.is_sparse
        outputs = []
        if self.prompts_condition ==True:
            x = self.add_prompt(x, zero_matrix)

        if self.residual:
            x = self.in_linear(x).to(self.device)
